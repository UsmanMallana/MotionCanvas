...
# List to collect recent shapes for combinations (up to 4)
recent_shapes = []
...
def get_shape_prediction(stroke_coords):
    ...
    if confidence < 80:  # You can adjust this threshold
...
def is_thumbs_up(hand_landmarks):
    ...
    # Thumb extended upwards (tip higher than base, smaller y)
    thumb_extended = lm[4].y < lm[3].y < lm[2].y
    # Other fingers curled (tips lower than bases, larger y)
...
def create_chat_window():
    """
    Creates and runs the Tkinter chat window with modern bubble UI.
    """
    try:
        root = tk.Tk()
        ...
        canvas = tk.Canvas(main_frame, bg="#34495E", borderwidth=0, highlightthickness=0)
        ...
        init_frame = ttk.Frame(scroll_frame)
        init_label = ttk.Label(init_frame, text="Waiting for you to draw shapes...", 
                               font=("Helvetica", 11), foreground="#ECF0F1", background="#4A627A", 
                               padding=(10, 5), relief="raised")
        init_label.pack(anchor="w")
        init_frame.pack(fill=tk.X, pady=5, anchor="w")
        canvas.yview_moveto(1.0)

        def check_queue():
            """
            Polls the message queue and updates the chat log with bubbles.
            """
            try:
                while not message_queue.empty():
                    msg_type, *data = message_queue.get_nowait()
                    
                    if msg_type == 'detected':
                        shape, confidence = data
                        # Detection bubble (left-aligned, gray)
                        det_frame = ttk.Frame(scroll_frame)
                        det_label = ttk.Label(det_frame, text=f"You drew a {shape}! (Confidence: {confidence:.2f}%)", 
                                              font=("Helvetica", 11, "bold"), foreground="#3498DB", background="#4A627A", 
                                              padding=(10, 5), relief="raised")
                        det_label.pack(anchor="w")
                        det_frame.pack(fill=tk.X, pady=5, anchor="w")
                    
                    elif msg_type == 'combo':
                        message = data[0]
                        # Decoded message header (left)
                        dec_frame = ttk.Frame(scroll_frame)
                        dec_label = ttk.Label(dec_frame, text="Decoded Message:", 
                                              font=("Helvetica", 12, "bold"), foreground="#1ABC9C", background="#34495E")
                        dec_label.pack(anchor="w")
                        dec_frame.pack(fill=tk.X, pady=5, anchor="w")
                        
                        # Message bubble (right-aligned, green like WhatsApp)
                        bubble_frame = ttk.Frame(scroll_frame)
                        bubble_label = ttk.Label(bubble_frame, text=message, 
                                                 font=("Helvetica", 11), foreground="#ECF0F1", background="#1ABC9C", 
                                                 padding=(10, 5), relief="raised")
                        bubble_label.pack(anchor="e")
                        bubble_frame.pack(fill=tk.X, pady=5, anchor="e")
                        
                        # Play audio for the decoded message (placeholder: assume file exists)
                        try:
                            audio_file = f'messages/{message.lower().replace(" ", "_").replace("!", "").replace(".", "")}.mp3'
...
while returning_frame:
    ...
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            if is_thumbs_up(handslms):
                cv2.putText(frame, "Thumbs Up! Decoding...", (200, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                if recent_shapes:
                    combined_message = ' '.join([hidden_messages.get(s, hidden_messages['unknown']) for s in recent_shapes])
                    message_queue.put(('combo', combined_message))
                    recent_shapes = []  # Clear after decoding
                        ...
                        if curr_index > 0 and len(strokes[curr_index-1]) > 5: # Check if there's anything to predict
                            try:
                                ...
                                if shape and shape != 'unknown':
                                    recent_shapes.append(shape)
                                    message_queue.put(('detected', shape, confidence))
                                    if len(recent_shapes) > 4:
                                        recent_shapes.pop(0)  # Keep only last 4
                            except Exception as e:
