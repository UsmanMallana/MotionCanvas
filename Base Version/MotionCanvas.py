...
def roi_from_coords(coords, padding=25):
    img = np.zeros((480, 640, 1), dtype=np.uint8) + 255
    width = 640
    height = 480
    coords = [(x,y) for x, y, c in coords]
    for i in range(len(coords)-1):
        pt1 = coords[i]
        pt2 = coords[i+1]
        cv2.line(img, pt1,pt2, color=0, thickness=20)
    ...
    cv2.imwrite('Image.png',roi_resized)
            ...
            if menu_active == False:
                if index_finger_straight and middle_finger_straight and ring_finger_straight and thumb_straight and small_finger_straight:
                    ...
                    if paused == True:
                        ...
                        curr_index += 1
                        paused = False
                        #get_digit_prediction(strokes[-2])
